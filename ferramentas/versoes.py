...
2. Escreve `versao.json` e o `<meta name="emdia-versao">` de cada página,
...
META = re.compile(r'<meta name="emdia-versao" content="(?P<versao>[^"]*)"\s*/?>')
...
META_RESUMO = re.compile(r'\s*<meta name="emdia-versao"[^>]*>')
    ...
    for html in paginas:
        ...
        if atual:
            if atual.group('versao') != versao_site:
                problemas += 1
                texto = META.sub(
                    f'<meta name="emdia-versao" content="{versao_site}" />', texto, count=1)
        else:
            problemas += 1
            print(f'  sem meta de versão: {html.name}')
            texto = texto.replace(
                '<meta name="viewport"',
                f'<meta name="emdia-versao" content="{versao_site}" />\n  <meta name="viewport"',
                1)
        textos[html] = texto
